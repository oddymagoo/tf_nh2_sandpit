...
resource "aws_wafv2_web_acl" "backend" {
  ...
  description = "WebACL associated to Mobility Collaboration Backend Infrastructure"
  scope       = "REGIONAL"
  ...
  rule {
    ...
    visibility_config {
      cloudwatch_metrics_enabled = true
      metric_name                = "Test-${var.environment}-anonymousiplist-metrics"
      sampled_requests_enabled   = true
    }
  }
...
resource "aws_wafv2_web_acl_logging_configuration" "backend" {
  log_destination_configs = [aws_cloudwatch_log_group.backend.arn]
  resource_arn            = aws_wafv2_web_acl.backend.arn
}

resource "aws_cloudwatch_log_resource_policy" "backend" {
  policy_name = "aws-waf-logs-mobility-collaboration"
